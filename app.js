...
// 39 Publicaciones Reales de Mercado Libre con Precios Estimados en $ ARS
...
function recalculateMetrics() {
  ...
  state.items.forEach(item => {
    ...
    // Clasificación Pareto ABC (A: Agenda / Top sales, B: Cuentos populares, C: General)
...
function renderFinancialKpis() {
  // Cálculo de Valor Total de Inventario en $ ARS
  const totalValuation = state.items.reduce((acc, i) => acc + (i.stock * (i.price || 25000)), 0);
  const estimatedNetRevenue = Math.round(totalValuation * 0.87); // 13% comisión promedio MeLi
...
function renderCharts() {
  ...
  if (canvasTop) {
    ...
    const dataValuation = topItems.map(i => Math.round((i.stock * (i.price || 25000)) / 1000)); // en miles $
